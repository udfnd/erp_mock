...
import { cssObj as employmentCssObj } from '@/domain/gigwan/section/EmploymentCategories/styles';
...
export function JaewonCategorySangtaeSection({ jojikNanoId }: JaewonCategorySangtaeSectionProps) {
  ...
  const hasEmptySangtae = useMemo(
    () => categories.some((category) => category.sangtaes.some((sangtae) => sangtae.name.trim().length === 0)),
    [categories],
  );
  ...
  return (
    <section css={employmentCssObj.card}>
      <div css={employmentCssObj.cardHeader}>
        <div css={employmentCssObj.cardTitleGroup}>
          <h2 css={employmentCssObj.cardTitle}>재원 상태 카테고리 상태</h2>
          <p css={employmentCssObj.cardSubtitle}>사용중인 카테고리는 수정하거나 삭제할 수 없어요</p>
        </div>
      </div>

      <div css={employmentCssObj.cardBody}>
          ...
          {(categoriesField) => (
            <>
              {categoriesField.state.value.map((category, categoryIndex) => (
                <div key={category.nanoId} css={employmentCssObj.categorySection}>
                  <span css={employmentCssObj.categoryLabel}>{category.name}</span>
                  <form.Field name={`categories[${categoryIndex}].sangtaes`} mode="array">
                    {(sangtaesField) => (
                      <div css={employmentCssObj.statusList}>
                        {sangtaesField.state.value.map((sangtae, sangtaeIndex) => (
                          ...
                          >
                            {(field) => {
                              ...
                              const localId = currentSangtae?.localId ?? `${category.nanoId}-${sangtaeIndex}`;
                              const isEditing = Boolean(editingMap[localId]);
                              const value = String(field.state.value ?? '');
                              return (
                                <div css={employmentCssObj.statusField} role="group">
                                  {isEditing ? (
                                    <input
                                      css={employmentCssObj.statusInputField}
                                      value={value}
                                      onChange={(event) => {
                                    ...
                                    <span css={employmentCssObj.statusValue}>
                                      {value.trim() || '새 상태'}
                                    </span>
                                  )}
                                  <div css={employmentCssObj.statusActions}>
                                    <IconButton
                                      styleType="normal"
                                      size="small"
      ...
      <footer css={employmentCssObj.cardFooter}>
        <Button
          size="small"
          styleType="solid"
          variant="primary"
