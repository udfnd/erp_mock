...
function IconSelect({
  ...
  const normalizedOptions = useMemo(
    () => [{ label: '아이콘 없음', value: 'none' }, ...options.filter((option) => option.value !== 'all')],
    [options],
  );

  return (
    <select css={oebuLinkListViewCss.toolbarSelect} value={value} onChange={(event) => onChange(event.target.value)}>
...
function CreateOebuLinkPanel({ jojikNanoId, iconOptions, onAfterMutation, onExit }: CreatePanelProps) {
    ...
    <>
      ...
      <form id={formId} css={oebuLinkListViewCss.panelBody} onSubmit={handleSubmit}>
        ...
        <div css={oebuLinkListViewCss.buttonRow}>
          {onExit && (
            <Button type="button" onClick={onExit} disabled={isSaving} size="medium" hierarchy="secondary">
              취소
            </Button>
          )}
          <Button type="submit" loading={isSaving} size="medium">
            저장
          </Button>
        </div>
      </form>
...
function SingleSelectionPanel({ oebuLinkNanoId, oebuLinkName, iconOptions, onAfterMutation }: SingleSelectionPanelProps) {
  const { data: detailData } = useGetOebuLinkDetailQuery(oebuLinkNanoId, { enabled: Boolean(oebuLinkNanoId) });
  ...
  const [name, setName] = useState(detailData?.name ?? '');
  const [titleName, setTitleName] = useState(detailData?.titleName ?? '');
  const [linkUrl, setLinkUrl] = useState(detailData?.linkUrl ?? '');
  const [linkIconNanoId, setLinkIconNanoId] = useState(detailData?.linkIcon ?? 'none');
  const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false);

  useEffect(() => {
    setName(detailData?.name ?? '');
    setTitleName(detailData?.titleName ?? '');
    setLinkUrl(detailData?.linkUrl ?? '');
    setLinkIconNanoId(detailData?.linkIcon ?? 'none');
  }, [detailData?.linkIcon, detailData?.linkUrl, detailData?.name, detailData?.titleName]);
  ...
  return (
    <>
      ...
      <form id={formId} css={oebuLinkListViewCss.panelBody} onSubmit={handleSubmit}>
        ...
        <div css={oebuLinkListViewCss.panelSection}>
          <label css={oebuLinkListViewCss.panelLabel}>아이콘</label>
          <IconSelect value={linkIconNanoId ?? 'none'} onChange={setLinkIconNanoId} options={iconOptions} />
        </div>
        <div css={oebuLinkListViewCss.buttonRow}>
          <Button type="submit" loading={isSaving} size="medium">
            저장
  ...
  return (
    <>
      <div css={oebuLinkListViewCss.panelHeader}>
        ...
        <p css={oebuLinkListViewCss.panelSubtitle}>선택된 항목 중 하나를 클릭하여 상세 정보를 확인하세요.</p>
      </div>
  ...
  if (!jojikNanoId) {
    return (
      <aside css={oebuLinkListViewCss.settingsPanel}>
        ...
        <div css={oebuLinkListViewCss.panelBody}>
          <p css={oebuLinkListViewCss.helperText}>조직 ID가 없으면 외부 링크 데이터를 불러올 수 없습니다.</p>
        </div>
      </aside>
    );
  }
    ...
    return (
      <aside css={oebuLinkListViewCss.settingsPanel}>
        <div css={oebuLinkListViewCss.panelHeader}>
          ...
          <p css={oebuLinkListViewCss.panelSubtitle}>외부 링크를 보거나 수정하려면 인증이 필요합니다.</p>
        </div>
        <div css={oebuLinkListViewCss.panelBody}>
